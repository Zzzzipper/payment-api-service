...
func (a Authenticator) Authenticate(ctx context.Context) (newCtx context.Context, err error) {
	if strings.ToLower(os.Getenv("USE_AUTH")) == "false" {
		err = nil
		newCtx = context.WithValue(purgeHeader(ctx, "authorization"), userMDKey{},
			&UserMetadata{
				ID: "dummy",
			})
		return
	...
	defer func() {
		if err == nil {
			newCtx = context.WithValue(newCtx, userMDKey{},
				&UserMetadata{
					ID: a.Username,
				})
		}
